...
const ProfilePage = () => {
  ...
  const [user, setUser] = useState({
    ...
    "storedFilesIds": null,
    "authorities": [],
    "img_url": "https://v1910509.hosted-by-vdsina.ru/files/zheenbek_akimzhanov-ac407020-68e3-4863-8451-bb7319ce3f1c.png"
  });
    ...
    <div className="">
      <div className="flex flex-col items-center justify-center  p-10 text-center">
        <div className="w-32 h-32 rounded-full overflow-hidden border-2 border-white bg-gray-300">
          <img
            src={user.img_url ? user.img_url : "https://t4.ftcdn.net/jpg/02/17/34/67/360_F_217346796_TSg5VcYjsFxZtIDK6Qdctg3yqAapG7Xa.jpg"}
            alt="Profile"
        ...
        <div className="mt-5">
          ...
          <div className="flex mt-3">
            <div className="mx-3">
              <p className="font-bold text-2xl">{user.followersIds.length}</p>
              <p className="text-gray-500">Followers</p>
            </div>
            <div className="mx-3">
              <p className="font-bold text-2xl">{user.followingsIds?.length}</p>
              <p className="text-gray-500">Following</p>
            </div>
            <div className="mx-3">
              <p className="font-bold text-2xl">{user.takenCoursesIds?.length}</p>
              <p className="text-gray-500">Taken courses</p>
            </div>
            <div className="mx-3">
              <p className="font-bold text-2xl">{user.publishedCoursesIds?.length}</p>
            ...
            <div className="profile-actions">
              {user.followersIds?.includes(currentUser.id) ? (
                ...
                >
                  ...
                  <button className="mr-3 px-4 py-2 rounded-full bg-green-500 text-white" onClick={handleFollow}>
                  Follow
                </button>
              )}

              <button className="px-4 py-2 rounded-full bg-blue-500 text-white" onClick={handleMessage}>
                Message
              </button>
            </div>
          )}
